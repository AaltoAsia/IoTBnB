...
angular.module('IoTBnB')
  .service('omiMessage', function ($http) {
    ...
    this.send = function (method, request, params, loadingBar, url) {
      params = params || {};
      ...
      var options = {
        ...
        headers: {'Content-Type': 'text/xml'},//XMLHttpRequest/xml
    ...
    this.restApi = function (url, isInfoItem) {
      if (!pendingRequests[url]) {
        //console.log(url)
        var promise = $http.get(url, {headers: {'Content-Type': 'text/xml'}})//XMLHttpRequest
          .then(function(response) {
            if (isInfoItem) {
